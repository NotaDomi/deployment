  ...
  const [message,setMessage] = useState('')
    ...
    useEffect( () => {
      ...
      const sendMessage = (event) => { 
        ...
        axios.post('https://notagram-api.onrender.com/api/messages/send', {
            sender: loggedUser.id,
            receiver: friend.id,
            content: message
        }).then( () => {
            axios.get(`https://notagram-api.onrender.com/api/messages/getMessages/${loggedUser.id}/${friend.id}`).then( res => {
