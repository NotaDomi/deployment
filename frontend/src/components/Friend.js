...
export default function Friend({username, myId, setLogged, loggedUser, setLoggedUser,setMessages,setClick,setFriend,allFriends,setFriends,setIsButtonDisabled,isButtonDisabled}) {
  ...
  const removeFriend = () => {
    ...
    axios.post('https://notagram-api.onrender.com/api/users/removeFriend?_method=PUT', {
        id: myId,
        username: username
    }).then( res => {
...
const apriChat = () => {
  axios.get(`https://notagram-api.onrender.com/api/messages/getMessages/${loggedUser.id}/${myId}`).then( res => {
